...
using Poliza.Models.ListaSelectorPolizaEstado;
using System.Net;
using System.Reflection;

namespace Poliza.Services.ListaSelectorPolizaEstado
{
    public class ListaSelectorPolizaEstadoService
    {
        private readonly ILogger<ListaSelectorPolizaEstadoService> _logger;
        private readonly DbContextFactoryService _dbContextFactoryService;

        public ListaSelectorPolizaEstadoService(ILogger<ListaSelectorPolizaEstadoService> logger, DbContextFactoryService dbContextFactoryService)
        {
            _logger = logger;
            _dbContextFactoryService = dbContextFactoryService;
        }

        public async Task<IActionResult> ListaSelectorPolizaEstado(HttpContext httpContext)
        {
            try
            {
                ...
                List<ListaSelectorPolizaEstadoResponseModel> registros = new();

                using (var dbContext = _dbContextFactoryService.CreateDbContext<PolizaDbContext>())
                {
                    var query = dbContext.PolizaEstado.Where(x => !x.EstaEliminado);
                    ...
                    registros = await query
                         .Take(10)
                         .Select(x => new ListaSelectorPolizaEstadoResponseModel
                         {
                             Id = x.Id,
            ...
            {
                BPSegurosException argosEx = new BPSegurosException((int)HttpStatusCode.InternalServerError, "Ha ocurrido un error al obtener los estados de las pólizas del selector.", ex);
