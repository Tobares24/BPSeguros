...
using Poliza.Models.ListaSelectorPolizaCobertura;
using System.Net;
using System.Reflection;

namespace Poliza.Services.ListaSelectorPolizaCobertura
{
    public class ListaSelectorPolizaCoberturaService
    {
        private readonly ILogger<ListaSelectorPolizaCoberturaService> _logger;
        private readonly DbContextFactoryService _dbContextFactoryService;

        public ListaSelectorPolizaCoberturaService(ILogger<ListaSelectorPolizaCoberturaService> logger, DbContextFactoryService dbContextFactoryService)
        {
            _logger = logger;
            _dbContextFactoryService = dbContextFactoryService;
        }

        public async Task<IActionResult> ListaSelectorPolizaCobertura(HttpContext httpContext)
        {
            try
            {
                ...
                List<ListaSelectorPolizaCoberturaResponseModel> registros = new();

                using (var dbContext = _dbContextFactoryService.CreateDbContext<PolizaDbContext>())
                {
                    var query = dbContext.PolizaCobertura.Where(x => !x.EstaEliminado);
                    ...
                    registros = await query
                         .Take(10)
                         .Select(x => new ListaSelectorPolizaCoberturaResponseModel
                         {
                             Id = x.Id,
            ...
            {
                BPSegurosException argosEx = new BPSegurosException((int)HttpStatusCode.InternalServerError, "Ha ocurrido un error al obtener las coberturas del selector.", ex);
